...
nlsy_kids <- 
  nlsy_kids %>%
  mutate(std_5to6 = (comp_score_5to6 - mean(comp_score_5to6, na.rm = TRUE))/sd(comp_score_5to6, na.rm = TRUE),
         std_5to6 = (comp_score_7to10 - mean(comp_score_7to10, na.rm = TRUE))/sd(comp_score_7to10, na.rm = TRUE),
         std_5to6 = (comp_score_11to14 - mean(comp_score_11to14, na.rm = TRUE))/sd(comp_score_11to14, na.rm = TRUE))
# which generates exactly the same variables.

# Now we run a FE regression of each standardized score on HS participation,  
# finding that the estimated effects shrink as children get older. HS raises 
...
# covariates. The effect on the test score at age 5-6 is still largest, although
# the relative effect sizes at 7-10 and 11-14 are flipped.
nlsy_kids_subsample <-
  nlsy_kids %>%
...
# the government should expand or cut funding for this and similar programs depends
# on its cost-effectiveness compared with other potential uses of funds. In general,
...
# same time, the lack of significant treatment effect heterogeneity in Problem 8 
