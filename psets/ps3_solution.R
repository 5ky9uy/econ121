...
options(scipen=999)
# this is not important, I just hate looking at scientific notation!
...
# It is possible to use the hypotheses() function with coefficients on  
...
# richer, more-educated Black group. The difference is
...
# I use the logit model again, and I exponentiated the  coefficients for 
# interpretability. I control for insurance status, smoking status, exercise, 
# bacon consumption, and obesity. To keep the samples the same in the 
# regressions with and without the additional control variables, I run 
# the long regression and the short regression on the same sample, which
# required subsetting the data first. This was not required.

# Smoking, exercise, and obesity predicted mortality: ever smoking doubled 
# the odds of death, ever exercising reduced the odds by 38%, ever binge 
# drinking raised the odds by 20%, and obesity raised the odds by 17%. 
# In contrast, uninsurance did not significanty associate with mortality. 
# The patterns explain part of the socioeconomic gradient in health. After 
...
# education dummy rose from 0.65 to 0.87. Because the odds ratios are  
